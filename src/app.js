...
// Fetch destinations from Supabase (destinations table)
async function loadCities() {
  ...
  try {
    ...
    // Map columns from Supabase schema
    cities = (data || []).map(row => ({
      id: row.id,
      name: row.name,
      country: row.country,
...
// Activities are now fetched from Supabase (destination_activities table)
// Cache to avoid repeated network requests
const _activitiesCache = {};
let _allActivitiesCache = null;

// Fetch activities for a specific city from Supabase
async function fetchActivitiesByCity(cityId) {
  if (_activitiesCache[cityId]) return _activitiesCache[cityId];
  ...
  try {
    ...
    _activitiesCache[cityId] = mapped;
    return mapped;
  } catch (err) {
...
// Fetch a single activity by its ID from Supabase
async function fetchActivityById(activityId) {
  // Check cache first
  for (const cityId in _activitiesCache) {
    const found = _activitiesCache[cityId].find(a => a.id === activityId);
...
async function getActivitiesByCity(cityId) {
  return await fetchActivitiesByCity(cityId);
}
...
function getCO2Label(co2) {
  if (co2 < 50) return 'ðŸŸ¢';
  if (co2 < 150) return 'ðŸŸ¡';
  return 'ðŸ”´';
}
